...
public class CustomFoodDetailActivity extends Activity implements Runnable {
	...
	private boolean addRecordToJournal() {
		...
		// DiaryDbHelper.getDiaryDbHelper(getApplicationContext());
		/*
	...
	private AlertDialog buildChangeAmountDialog(String amount) {
		...
		alert.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int whichButton) {
				...
				mEntry.put(CustomFoodPointsDiaryTableHelper.COL_NUM_SERVINGS,
						amount);
	...
	private void fillData(ContentValues entry,
		...
		final TextView srvg_desc = (TextView) findViewById(R.id.servingNote_TextView);
		srvg_desc.setText(entry
				.getAsString(CustomFoodPointsDiaryTableHelper.COL_COMMENT));
		updateValuesOnServingChange();
	}
	...
	private void fillPointViz(ContentValues entry,
		...
		for (int i = 0; i < COMPONENT_ORDER.size(); i++) {
			...
			final PointComponent curGoalComponent = COMPONENT_ORDER.get(i);
			...
			if (entry.containsKey(curGoalComponent.getPtDbColName())) {
				baseNutVal = entry
						.getAsFloat(curGoalComponent.getPtDbColName());
			}
			...
			if (goals.containsKey(curGoalComponent)) {
				fillValues(list_holder, curGoalComponent,
						diary.get(curGoalComponent), adjustedNutVal,
						goals.get(curGoalComponent));
			} else {
				fillValues(extra_list_holder, curGoalComponent,
						diary.get(curGoalComponent), adjustedNutVal, 10);
			}
	...
	private void loadData() {
		...
		mEntry = mDiaryHelper.getCustomFoodPointsEntryAsVals(id);// mDbHelper.getNutritionEntry((int)
																	// id);
	...
	protected Dialog onCreateDialog(int id) {
		...
		switch (id) {
		case DIALOG_AMOUNT:
			dialog = buildChangeAmountDialog(mServingAmountValue_EditText
					.getText().toString());
			break;
		default:
			dialog = null;
