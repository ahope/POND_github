...
public class StudyOverviewActivity extends OverviewActivity {
	...
	private static final String[][] CONDITION_ORDER = { { "F", "S", "B", "M" },
			{ "S", "M", "F", "B" }, { "M", "B", "S", "F" },
			{ "B", "F", "M", "S" } };
	private static final String[] TASK_ORDER = { "P1", "P2", "B1", "SS1", "L1",
			"BS1", "D1", "P1", "P2", "B2", "SS2", "L2", "BS2", "D2", "P1",
			"P2", "B3", "SS3", "L3", "BS3", "D3", "P1", "P2", "B4", "SS4",
			"L4", "BS4", "D4" };
	...
	private String clearGoalsFromMap(HashMap<PointComponent, Double> results,
			int whichCond) {
		...
		final String[] goals = newGoals.split(",");

		// Go through the map, setting all other goals to -1
		final Iterator<PointComponent> iter = results.keySet().iterator();
		while (iter.hasNext()) {
			final PointComponent pc = iter.next();
			boolean found = false;
		...
		// mDiaryHelper.logAction(Action.STUDY_TASK, -1, -1,
		// "Task result heading", headings);
		// String trimmedResults = results.toString().replaceAll("[{}[A-Z]_=]",
		// "");
		// mDiaryHelper.logAction(Action.STUDY_TASK_RESULT, -1, -1,
		// "Task result", getCurrentInfoString() + "," + trimmedResults);
	...
	private void endTask() {
		mDiaryHelper.logAction(Action.STUDY_TASK, -1, curPptNum,
				"Completed task: " + curTaskId, getCurrentInfoString());
		final Date endTime = new Date();
		final HashMap<Consts.PointComponent, Double> results = mDiaryHelper
				.getSummedPointsEntryAsMapForTimeFrame(curTaskStartTime,
						endTime);
		final String headings = results.toString().replaceAll("[.=[0-9]]", "");
		mDiaryHelper.logAction(Action.STUDY_TASK, -1, curPptNum,
				"Task result heading", headings);

		clearGoalsFromMap(results, curCondIndex);

		final String trimmedResults = (results.toString()).replaceAll(
				"[{}[A-Z]_=]", "");

		mDiaryHelper.logAction(Action.STUDY_TASK_RESULT, -1, curPptNum,
				"Task result", getCurrentInfoString() + "," + trimmedResults);
		mDiaryHelper.logAction(Action.STUDY_TASK_TIME, -1, curPptNum,
				"Task time", getCurrentInfoString() + ","
						+ (endTime.getTime() - curTaskStartTime.getTime()));
	}
	...
	private Map<PointComponent, Integer> getNewGoalMap(String goals) {
		final Map<PointComponent, Integer> map = new HashMap<PointComponent, Integer>();

		final String[] items = goals.split(",");
	...
	private List<PointComponent> getNewPointComponentList() {
		final List<PointComponent> comps = new ArrayList<PointComponent>();
	...
	private String getRandomOrdering(String condId) {
		...
		final StringBuffer buffer = new StringBuffer(condId);
		...
		if (condId.equals("F")) {
			...
			if (components.size() < num) {
				final List<PointComponent> newBatch = getNewPointComponentList();

				final Iterator<PointComponent> existingIter = components
						.iterator();
	...
	public void onBackPressed() {
		if (!preppingForStudyStart) {
			...
			if (curPptId.equals("-1")) {
				mDiaryHelper.logAction(Action.STUDY, -1, -1,
						"Study not running", "");
				Toast.makeText(getApplicationContext(), "Study not running",
				...
				final AlertDialog.Builder builder = new AlertDialog.Builder(
						this);
				builder.setMessage(
						"Are you sure you're done with " + curTaskId + "?")
						.setCancelable(false)
						.setPositiveButton("Yes",
								new DialogInterface.OnClickListener() {
									...
									public void onClick(DialogInterface dialog,
										...
										if (curTaskIndex >= TASK_ORDER.length) {// There
																				// are
																				// no
																				// more
																				// tasks
																				...
																				// 4){
											// Study is over
											mDiaryHelper.logAction(
													Action.STUDY, -1, -1,
													"Study is over",
													getCurrentInfoString());
											final SharedPreferences settings = getSharedPreferences(
													STUDY_PREFS_NAME, 0);
											...
											// If done with tasks in this
											// condition, move to the next
											// condition
											...
											final SharedPreferences.Editor editor = settings
													.edit();
											editor.putString(STUDY_PPT_ID_KEY,
													curPptId);
											editor.putString(
													STUDY_COND_ID_KEY,
													CONDITION_ORDER[curPptNum % 4][curCondIndex]);
											editor.putString(STUDY_TASK_ID_KEY,
													curTaskId);
											editor.putInt(STUDY_TASK_INDEX_KEY,
													curTaskIndex);
											editor.putInt(STUDY_PPT_NUM_KEY,
													curPptNum);
											editor.putInt(STUDY_COND_INDEX_KEY,
													curCondIndex);
											editor.commit();
	...
	public void onCreate(android.os.Bundle savedInstanceState) {
		...
		if (curPptId.equals("-1")) {
			...
			mDiaryHelper.logAction(Action.STUDY, -1, -1,
					"Creating Overview screen", getCurrentInfoString());
			mDiaryHelper.logAction(Action.STUDY_CONDITION, -1, -1,
					"Starting Condition: " + curCondId, getCurrentInfoString());
			mDiaryHelper.logAction(Action.STUDY_TASK, -1, -1, "Starting Task: "
					+ TASK_ORDER[curTaskIndex], getCurrentInfoString());
			Toast.makeText(getApplicationContext(),
					curCondId + ": " + TASK_ORDER[curTaskIndex],
					Toast.LENGTH_LONG).show();
		}
	...
	public boolean onSearchRequested() {
		...
		appData.putLong(FoodDetailActivity.DATA_DATETIME_EATEN_KEY,
				curCondStartTime.getTime());
	...
	private void startNextCondition() {
		...
		builder.create();
		// Change the goals
		mDiaryHelper.logAction(Action.STUDY_CONDITION, -1, curPptNum,
				"Condition is over", getCurrentInfoString());
		curCondIndex++;
		String newGoals = "";
		final SharedPreferences settings = getSharedPreferences(
				STUDY_PREFS_NAME, 0);
		...
		builder.setSingleChoiceItems(items, -1,
				new DialogInterface.OnClickListener() {
					...
					public void onClick(DialogInterface dialog, int item) {
						...
						editor.putString(STUDY_COND_ID_KEY,
								CONDITION_ORDER[item % 4][0]);
						editor.putString(STUDY_TASK_ID_KEY,
								TASK_ORDER[curTaskIndex]);
						...
						mDiaryHelper.logAction(Action.STUDY, -1, curPptNum,
								"Randomly chosen components", curPptId + ","
										+ goals);
