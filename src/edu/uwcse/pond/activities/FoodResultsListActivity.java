...
public class FoodResultsListActivity extends Activity {
	...
	private AlertDialog buildFilterDialog() {
		...
		final Cursor manCursor = mDbHelper
	...
	private void doMySearch(final long locid, final Spinner food_class_spinner,
		...
		if (useClass.isChecked()) {
			...
			final Cursor c = mDbHelper.getFoodsWithName(mDialogQuery,
					(FoodClass) obj);
			fillData(c);
		} else {
			final Cursor c = mDbHelper.getFoodsWithName(mDialogQuery);
	...
	private void doMySearch(String query) {
		...
		final Cursor c = mDbHelper.getFoodsWithName(query);
	...
	private void fillData(Cursor c) {
		...
		list_view.setOnItemClickListener(new OnItemClickListener() {
			...
			public void onItemClick(AdapterView<?> arg0, View arg1, int arg2,
					long id) {
				final Intent intent = new Intent(FoodResultsListActivity.this,
	...
	public void onCreate(Bundle savedInstanceState) {
		 ...
		 * final AlertDialog.Builder alert = new AlertDialog.Builder(this);
		 * 
		 * LayoutInflater inflater = getLayoutInflater(); //FrameLayout f1 =
		 * (FrameLayout)alert.findViewById(android.R.id.body);
		 * //f1.addView(inflater.inflate(R.layout.dialog_view, f1, false)); View
		 * layout = inflater.inflate(R.layout.food_query_dialog, null);
		 * 
		 * final Spinner food_class_spinner =
		 * (Spinner)layout.findViewById(R.id.food_query_category_Spinner);
		 * food_class_spinner.setAdapter(new ArrayAdapter<FoodClass>(this,
		 * android.R.layout.simple_list_item_1, FoodClass.values())); final
		 * EditText input =
		 * (EditText)layout.findViewById(R.id.food_query_EditText); final
		 * CheckBox useClass =
		 * (CheckBox)layout.findViewById(R.id.food_query_CheckBox);
		 * 
		 * alert.setView(layout);
		 * 
		 * alert.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
		 * public void onClick(DialogInterface dialog, int whichButton) {
		 * mDialogQuery = input.getText().toString().trim(); doMySearch(locid,
		 * food_class_spinner, useClass); }
		 * 
		 * });
		 * 
		 * alert.setNegativeButton("Cancel", new
