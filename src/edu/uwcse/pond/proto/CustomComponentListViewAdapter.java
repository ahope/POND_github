...
public class CustomComponentListViewAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup arg2) {
		...
		if (!minusOneButtonListeners.containsKey(keys[position])){
			minusOneButtonListeners.put(keys[position], new MinusOneButtonClickListener(position));
		}
		minusOneButton.setOnClickListener(minusOneButtonListeners.get(keys[position]));
		minusOneButton.setOnLongClickListener(minusOneButtonListeners.get(keys[position]));
...
//		populateGoalView(convertView.getContext(), tokenHolder,layout, diaryVals.get(keys[position]), goalVals.get(keys[position]) );
	...
	class PlusOneButtonClickListener implements OnClickListener, OnLongClickListener {
		...
		public void onClick(View v) {
			// Get existing amount
			double val = foodVals.get(keys[myPosition]);
			
			// Update amount
			foodVals.put(keys[myPosition], val + 1);
			
			CustomComponentListViewAdapter.this.notifyDataSetChanged();
			CustomComponentListViewAdapter.this.context.updateComponentPlusOne(keys[myPosition]);
			/*
			CharSequence text = "+1 Added";
			int duration = Toast.LENGTH_SHORT;

			Toast toast = Toast.makeText(CustomComponentListViewAdapter.this.context, text, duration);
			toast.show();
			*/
		}
		...
		public boolean onLongClick(View arg0) {
			...
			CustomComponentListViewAdapter.this.notifyDataSetChanged();
			CustomComponentListViewAdapter.this.context.updateComponentPlusHalf(keys[myPosition]);
			/*
	...
	class MinusOneButtonClickListener implements OnClickListener, OnLongClickListener {
		...
		public void onClick(View v) {
			...
			if (val >= 1.0){
				...
				CustomComponentListViewAdapter.this.notifyDataSetChanged();
				CustomComponentListViewAdapter.this.context.updateComponentMinusOne(keys[myPosition]);
				/*
				CharSequence text = "-1";
				int duration = Toast.LENGTH_SHORT;
		...
		public boolean onLongClick(View arg0) {
			...
			if (val >= 0.5){
				...
				CustomComponentListViewAdapter.this.notifyDataSetChanged();
				CustomComponentListViewAdapter.this.context.updateComponentMinusHalf(keys[myPosition]);
				/*
				CharSequence text = "+1/2 Added";
				int duration = Toast.LENGTH_SHORT;
