...
public class CustomFoodDetailActivity extends Activity implements Runnable{
	...
	private ContentValues mEntry; //NutritionEntry mEntry; 
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.custom_food_nutrition_info_card);
        
        mProgressDialog = ProgressDialog.show(CustomFoodDetailActivity.this, "", 
	...
	private void loadData() {
        ...
        mGoals = mDiaryHelper.getMostRecentGoalAsMap();
        ...
        mEntry = mDiaryHelper.getCustomFoodPointsEntryAsVals(id);//mDbHelper.getNutritionEntry((int) id);
        
        mDiaryHelper.logAction(Action.VIEW_FOOD_DETAIL, 
        		(int)id, -1, "View food detail", 
        		mEntry.getAsString(CustomFoodPointsDiaryTableHelper.COL_NAME));
...
*/
	...
	//			Double.parseDouble(mServingAmountValue_EditText.getText().toString()));
		...
		mDiaryHelper.createNewPointsEntryFromCustomFood(mEntry);
		//diary_db.doneWithDb();
...
//aha        startActivity(intent);
	...
	private void fillData(ContentValues entry, 
         ...
         nameView.setText(entry.getAsString(CustomFoodPointsDiaryTableHelper.COL_NAME));
       ...
       /*  mServingType_Spinner.setOnItemSelectedListener(new OnItemSelectedListener() {
         ...
         mServingAmountValue_EditText.setText("1.0");
        ...
        TextView srvg_desc = (TextView)findViewById(R.id.servingNote_TextView);
        srvg_desc.setText(entry.getAsString(CustomFoodPointsDiaryTableHelper.COL_COMMENT));
        updateValuesOnServingChange();
    }

	private void fillPointViz(ContentValues entry,
			Map<PointComponent, Integer> goals,
			Map<PointComponent, Double> diary, 
     ...
     //    Iterator<PointComponent> entryComponents = entry.getPointsMap().keySet().iterator(); 
         ...
         for (int i=0; i<COMPONENT_ORDER.size(); i++){
        	 ...
        	 PointComponent curGoalComponent = COMPONENT_ORDER.get(i);
        	 
        	 double adjustedNutVal = 0;
        	 
        	 float baseNutVal = 0.0f; 
        	 if (entry.containsKey(curGoalComponent.getPtDbColName()) ){
        		 baseNutVal = entry.getAsFloat(curGoalComponent.getPtDbColName());  	 
        	 }
        	 adjustedNutVal = (float)mServingMultiplier * baseNutVal;//entry.getPointsMap().get(curGoalComponent).floatValue();
	...
	private void updateValuesOnServingChange() {
		...
		mServingMultiplier = Double.parseDouble(amt);
	...
	protected Dialog onCreateDialog(int id) {
	    ...
	    switch(id) {
	    case DIALOG_AMOUNT:
	    	dialog = buildChangeAmountDialog(mServingAmountValue_EditText.getText().toString());
	    	break; 
	    default:
	        dialog = null;
	    }
	...
	private AlertDialog buildChangeAmountDialog(String amount) {
		...
		alert.setPositiveButton("Ok", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int whichButton) {
				...
				mEntry.put(CustomFoodPointsDiaryTableHelper.COL_NUM_SERVINGS, amount);
				updateValuesOnServingChange();
				dialog.dismiss();
			}
		});
